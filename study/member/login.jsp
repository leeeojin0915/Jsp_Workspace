...
<%
	...
	String id=request.getParameter("id");//클라이언트가 요청시 전송한 파라미터값을 얻는 메서드
										//매개변수로는 파라미터 변수명을 명시, 만일 html 이용할 경우 컴포넌트에 부여된 name값을 명시
	...
	//드라이브 로드
	//모든 driver jar파일은 javaEE기반의 스팩을 따라서 위치시켜야 한다.
	//스펙에 의하면 class 파일과 jar는 WEB-INF라는 대문자로 된 보안 디렉토링 위치시켜야 한다.
	//클래스의 위치는 WEB-INF/classes, jar는 WEB-INF/lib에 둬야 한다.
